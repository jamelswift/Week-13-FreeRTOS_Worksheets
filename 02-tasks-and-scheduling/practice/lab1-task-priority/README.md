...
Ans High Priority Task (Priority 5) ทำงานมากที่สุด
เพราะ FreeRTOS ใช้ระบบ Preemptive Scheduling — task ที่มี priority สูงกว่าจะได้สิทธิ์ใช้ CPU ก่อนเสมอ หากพร้อม (ready) ที่จะรัน

2. เกิด Priority Inversion หรือไม่? จะแก้ไขได้อย่างไร?
Ans อาจเกิด Priority Inversion ได้ในส่วนสาธิต Priority Inversion Demo

3. Tasks ที่มี priority เดียวกันทำงานอย่างไร?
Ans Tasks ที่มี priority เท่ากันจะทำงานแบบ Round-Robin Scheduling

4. การเปลี่ยน Priority แบบ dynamic ส่งผลอย่างไร?
Ans การเปลี่ยน priority แบบ dynamic (เช่น ใช้ vTaskPrioritySet()) ทำให้ scheduler ปรับลำดับการรัน task ใหม่แบบ real-time

5. CPU utilization ของแต่ละ priority เป็นอย่างไร?
Ans 
- High Priority ใช้ CPU มากสุด
- Medium Priority รองลงมา
- Low Priority ใช้ CPU น้อยสุดและมักจะ idle
