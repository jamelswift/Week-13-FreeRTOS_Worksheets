...
Ans Task จะอยู่ใน Running state เมื่อมันถูกเลือกโดย Scheduler ให้ครอบครอง CPU และกำลัง “ทำงานจริง” อยู่บนคอร์นั้นในขณะนั้น

2. ความแตกต่างระหว่าง Ready และ Blocked state คืออะไร?
Ans
- Ready: พร้อมทำงาน แต่ยังไม่ได้รับ CPU
- Blocked:รอเงื่อนไขบางอย่างก่อนจึงจะกลับมาทำงานได้

3. การใช้ vTaskDelay() ทำให้ task อยู่ใน state ใด?
Ans การใช้ vTaskDelay() จะทำให้ task เข้าสู่ Blocked state

4. การ Suspend task ต่างจาก Block อย่างไร?
Ans 
- การ Suspend task คือ “หยุดทำงานแบบไม่ขึ้นกับเวลา” โดยต้องใช้ API (vTaskSuspend())
- ส่วน Block เกิดขึ้นอัตโนมัติเมื่อ task รอ event หรือ delay

5. Task ที่ถูก Delete จะกลับมาได้หรือไม่?
Ans ไม่ได้
เมื่อ task ถูกลบ (vTaskDelete()) → หน่วยความจำของ stack และ Task Control Block (TCB) จะถูกคืนให้ระบบทันที
