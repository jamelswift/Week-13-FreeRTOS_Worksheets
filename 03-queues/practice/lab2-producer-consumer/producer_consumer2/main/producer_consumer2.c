...
#include <string.h>
#include <stdlib.h>
#include <stdarg.h>            // ★ สำหรับ va_list/va_start/va_end
...
#define LED_PRODUCER_4 GPIO_NUM_23   // ★ เพิ่ม Producer4 (ปรับได้ตามบอร์ด)
...
void producer_task(void *pvParameters) {
    ...
    switch (producer_id) {
        ...
        case 4: led_pin = LED_PRODUCER_4; break;   // ★ รองรับ Producer4
...
void statistics_task(void *pvParameters) {
    ...
    while (1) {
        ...
        // Visual queue representation (10 slots)
        printf("Queue: [");
        for (int i = 0; i < 10; i++) {
...
void load_balancer_task(void *pvParameters) {
    ...
    while (1) {
        ...
        if (queue_items > MAX_QUEUE_SIZE) {
            ...
            gpio_set_level(LED_PRODUCER_4, 1);  // ★ รวม Producer4
            ...
            gpio_set_level(LED_PRODUCER_4, 0);  // ★ รวม Producer4
...
void app_main(void) {
    ...
    gpio_set_direction(LED_PRODUCER_4, GPIO_MODE_OUTPUT); // ★ ตั้งค่า Producer4
    ...
    gpio_set_level(LED_PRODUCER_2, 0);
    gpio_set_level(LED_PRODUCER_3, 0);
    gpio_set_level(LED_PRODUCER_4, 0); // ★ ปิดไฟ Producer4
    gpio_set_level(LED_CONSUMER_1, 0);
    gpio_set_level(LED_CONSUMER_2, 0);
    ...
    if (xProductQueue != NULL && xPrintMutex != NULL) {
        ...
        // Producer/Consumer IDs (ต้องเป็น static/global เพื่อไม่ให้ pointer ชี้ไปที่ stack ที่หมดอายุ)
        static int producer1_id = 1, producer2_id = 2, producer3_id = 3, producer4_id = 4; // ★ เพิ่ม 4
        ...
        xTaskCreate(producer_task, "Producer4", 3072, &producer4_id, 3, NULL); // ★ เพิ่ม Producer4
