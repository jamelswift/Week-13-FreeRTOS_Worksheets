...
#define LED_PRODUCER_4 GPIO_NUM_23   // ปรับได้ตามบอร์ด
...
void app_main(void) {
    ESP_LOGI(TAG, "Producer-Consumer System Lab Starting... (Experiment 3: Fewer Consumers)");
    ...
    if (xProductQueue != NULL && xPrintMutex != NULL) {
        ...
        // static int consumer2_id = 2;   // ทดลองที่ 3: ลดผู้บริโภค ปิดการใช้งาน Consumer 2
        ...
        // Create consumer tasks (เหลือแค่ Consumer1)
        xTaskCreate(consumer_task, "Consumer1", 3072, &consumer1_id, 2, NULL);
        // xTaskCreate(consumer_task, "Consumer2", 3072, &consumer2_id, 2, NULL); // ← ปิดตามโจทย์
