...
Ans Binary Semaphore เก็บค่าได้เพียง 0 หรือ 1 เท่านั้น
หาก xSemaphoreGive() ถูกเรียกหลายครั้งต่อเนื่องโดยไม่มี xSemaphoreTake() ค่าจะยังคงเป็น 1 (พร้อมใช้งาน) — การ give ซ้ำ ๆ ไม่มีผลเพิ่ม count อีก

2. ISR สามารถใช้ `xSemaphoreGive` หรือต้องใช้ `xSemaphoreGiveFromISR`?
Ans ใช้ xSemaphoreGiveFromISR() เสมอใน ISR — ส่วน xSemaphoreGive() ใช้ใน context ปกติ (task level) เท่านั้น

3. Binary Semaphore แตกต่างจาก Queue อย่างไร?
Ans Semaphore ใช้ “ส่งสัญญาณ” ว่ามีเหตุการณ์เกิดขึ้น ส่วน Queue ใช้ “ส่งข้อมูลจริง” ระหว่าง Task
