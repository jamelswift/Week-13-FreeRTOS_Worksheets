...
Ans ระบบ Preemptive Multitasking มีเวลาตอบสนอง (response time) ดีกว่าและสม่ำเสมอกว่า
เพราะ RTOS จะ “แย่ง CPU” ให้กับ task ที่มี priority สูงทันที (เช่น emergency task) โดยไม่ต้องรอ task อื่น “ยอมให้”

2. ข้อดีของ Cooperative Multitasking คืออะไร?
Ans 
- ระบบเรียบง่าย ไม่ต้องมี interrupt จาก kernel มาบังคับ
- ใช้ทรัพยากร (RAM/CPU) น้อย เพราะไม่ต้องเก็บ context ซับซ้อน
- เข้าใจง่ายและ debug ง่าย
- ไม่มีปัญหา “race condition” ระหว่าง task เพราะแต่ละ task รันจนจบก่อนสลับ

3. ข้อเสียของ Cooperative Multitasking คืออะไร?
Ans
- ถ้า task หนึ่งไม่ยอม yield (เช่น loop หนักโดยไม่หน่วงเวลา) → ทั้งระบบจะ “ค้าง”

- เวลาตอบสนอง (response time) ไม่แน่นอน

- ไม่เหมาะกับงานที่ต้องการ reaction ทันที เช่น interrupt, emergency หรือ real-time control

4. ในสถานการณ์ใดที่ Cooperative จะดีกว่า Preemptive?
Ans 
- เหมาะในระบบที่ เรียบง่าย ไม่ต้องการ real-time strict

- ใช้ใน embedded systems ขนาดเล็ก, สื่อการสอน, หรือ simulation demo

- ใช้ได้ดีเมื่อทุก task ถูกออกแบบให้ “yield เป็นจังหวะ” อย่างมีวินัย

- ใช้ในระบบที่การสลับ context บ่อยไม่จำเป็น เช่น ระบบ logging, UI, หรือ data acquisition ที่คาบเวลาแน่นอน

5. เหตุใด Preemptive จึงเหมาะสำหรับ Real-time systems?
And เพราะ Preemptive scheduler ของ RTOS สามารถจัดลำดับความสำคัญ (priority scheduling)
และ interrupt task ที่สำคัญให้รันได้ทันที ตามเวลาที่กำหนด → จึงตอบสนองแบบ deterministic
