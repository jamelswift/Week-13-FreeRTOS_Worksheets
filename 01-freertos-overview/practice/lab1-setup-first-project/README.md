...
Ans
- CMakeLists.txt (root)	ระบุชื่อโปรเจกต์ และรวมระบบ build ของ ESP-IDF
- main/CMakeLists.txt	บอกว่าไฟล์ .c ไหนเป็น source code ของโปรเจกต์
- main/xxx.c	โค้ดหลักที่มี void app_main(void)
- sdkconfig เก็บการตั้งค่าคอนฟิกทั้งหมดของโปรเจกต์ (จะสร้างอัตโนมัติหลัง build ครั้งแรก)

2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
Ans 
.elf → สำหรับ debug / ตรวจสอบ symbol
.bin → สำหรับ เขียนลง Flash ของบอร์ด

3. คำสั่ง `idf.py set-target` ทำอะไร?
Ans เป็นคำสั่งบอก ESP-IDF ว่าจะ build สำหรับ บอร์ดรุ่นไหน เพื่อให้เลือก compiler และ library ที่ถูกต้อง

4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
Ans build/ คือโฟลเดอร์ผลลัพธ์ของการ compile และ link ทั้งหมด
→ เป็นที่รวมทุกไฟล์ที่ใช้ flash เข้า ESP32

5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
Ans vTaskDelay() เป็น “cooperative yield” ในระบบ multitasking → ทำให้ ESP32 ทำงานหลาย task พร้อมกันได้จริง
จึงเป็นวิธีที่ถูกต้องเมื่อทำงานใน FreeRTOS environment
## บทสรุป
