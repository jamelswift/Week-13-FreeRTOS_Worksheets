...
Ans เพราะ Task ใน FreeRTOS จะถูก Scheduler เรียกซ้ำเรื่อย ๆ เพื่อให้ทำงานแบบต่อเนื่อง
ถ้าไม่มี loop → task จะสิ้นสุดการทำงาน → ระบบจะคืน stack memory ทันที → task นั้น “หายไปจาก scheduler”

2. ความหมายของ stack size ใน xTaskCreate() คืออะไร?
Ans
คือ ขนาดหน่วยความจำ (stack) ที่จัดสรรให้ task นั้น ๆ สำหรับเก็บตัวแปรภายในและการเรียกฟังก์ชัน

3. ความแตกต่างระหว่าง vTaskDelay() และ vTaskDelayUntil()?
Ans
| ฟังก์ชัน                                    | การทำงาน                              | เหมาะสำหรับ                              |
| ------------------------------------------- | ------------------------------------- | ---------------------------------------- |
| **`vTaskDelay(ticks)`**                     | หน่วงเวลานับจาก “ตอนที่เรียกฟังก์ชัน” | การหน่วงแบบไม่คงคาบเวลา (relative delay) |
| **`vTaskDelayUntil(&lastWakeTime, ticks)`** | หน่วงเวลาแบบอ้างอิงจาก “รอบก่อนหน้า”  | การทำงานแบบคงคาบเวลา (periodic task)     |

4. การใช้ vTaskDelete(NULL) vs vTaskDelete(handle) ต่างกันอย่างไร?
Ans
- NULL → ใช้ภายในตัว task เอง
- handle → ใช้จาก task อื่นเพื่อลบ task เป้าหมาย

5. Priority 0 กับ Priority 24 อันไหนสูงกว่า?
Ans Priority 24 สูงกว่า Priority 0
